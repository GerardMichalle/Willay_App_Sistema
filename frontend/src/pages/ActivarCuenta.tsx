...
import {
  ArrowRight, ArrowLeft, Verified, KeyRound, Loader2,
...
export default function ActivarCuenta() {
  ...
  // Transición visual entre pasos (cross-fade con desplazamiento lateral).
  // Puramente presentacional: no cambia cuándo ni cómo avanza `paso`.
  const [pasoVisible, setPasoVisible] = useState<Paso>(paso);
  const [pasoSaliente, setPasoSaliente] = useState<Paso | null>(null);
  const pasoPrevio = useRef(paso);
  useEffect(() => {
    if (pasoPrevio.current === paso) return;
    setPasoSaliente(pasoPrevio.current);
    setPasoVisible(paso);
    pasoPrevio.current = paso;
    const t = setTimeout(() => setPasoSaliente(null), 400);
  ...
  /** Contenido interno de cada paso (sin el wrapper de animación, que se aplica en el sitio de uso). */
  function contenidoPaso(p: Paso) {
    if (p === 'codigo') {
      return (
        <>
          <h1 className="text-[18px] font-bold tracking-tight">Activa tu cuenta</h1>
          <p className="text-[12.5px] text-ink-3 mt-1 mb-5">
            Ingresa el código de 6 dígitos que te entregó la institución junto con tu DNI. El sistema te reconocerá automáticamente.
          </p>
          ...
          <p className="text-[11.5px] text-ink-3 mt-4 text-center">
            ¿No tienes un código? Pídelo en la secretaría de tu institución.
          </p>
        </>
      );
    }
  ...
  /** Clases del wrapper de cada paso: layout propio del paso + animación de entrada o salida. */
  function claseWrapperPaso(p: Paso, saliendo: boolean) {
    return cn(
      p === 'listo' && 'text-center py-4',
      saliendo ? 'animate-fade-slide-out' : (p === 'listo' ? 'animate-pop' : 'animate-fade-slide'),
    );
  }
      ...
      <div className="relative flex items-center justify-center px-4 py-10 overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-[360px] animate-rise">
          ...
          <div className="card p-7 shadow-[0_10px_40px_rgba(0,0,0,.06)] overflow-hidden">
            ...
            {/* Contenido del paso: cross-fade — el panel de marca nunca se mueve,
                solo este bloque transiciona al avanzar/retroceder de paso. */}
            <div className="relative">
              {pasoSaliente && (
                <div
                  key={`saliente-${pasoSaliente}`}
                  aria-hidden
                  className={cn('absolute inset-0 pointer-events-none', claseWrapperPaso(pasoSaliente, true))}
                >
                  {contenidoPaso(pasoSaliente)}
                </div>
              )}
              <div key={`actual-${pasoVisible}`} className={claseWrapperPaso(pasoVisible, false)}>
                {contenidoPaso(pasoVisible)}
              </div>
            </div>
          </div>
          ...
          <p className="flex items-center justify-center gap-1.5 label-mono mt-4">
            <Verified size={13} className="text-brand" /> Solo la institución puede emitir códigos
          </p>
        </div>
      </div>
    </div>
