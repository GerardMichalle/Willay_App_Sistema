...
const ESTADO_POR_TAB: Record<string, string | undefined> = {
  Activos: 'ACTIVO', Pendientes: 'PENDIENTE', Suspendidos: 'SUSPENDIDO',
};
...
export default function Usuarios() {
  ...
  const cargar = useCallback(async () => {
    ...
    setError(null);
    try {
      const r = await getUsuariosAdmin({ q: busqueda || undefined, estado: ESTADO_POR_TAB[tab], pagina, tamano: 50 });
  ...
  // Búsqueda con retardo: no golpea el backend en cada tecla (mismo patrón que Alumnos.tsx).
  useEffect(() => {
  ...
  // Cambiar de filtro o de búsqueda vuelve a la primera página.
  ...
  return (
    <>
      <Topbar
        title="Usuarios"
        subtitle={cargando ? 'Cargando…' : `${total} cuenta${total === 1 ? '' : 's'}`}
      />
